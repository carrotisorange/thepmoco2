<x-new-layout>
    @section('title','Step 2 of Step 4 | Unit Inventory | '. env('APP_NAME'))
    <div class="mx-auto px-4 sm:px-6 lg:px-8 xl:px-10 xl:py-10">
            ...
            <nav class="mx-auto max-w-9xl px-4 sm:px-6 lg:px-8" aria-label="Progress">
                <ol role="list"
                    ...
                    {{-- <li class="relative overflow-hidden lg:flex-1">
                        <div class="border border-gray-200 overflow-hidden border-t-0 rounded-b-md lg:border-0">
                            <!-- Upcoming Step -->
                            <a href="/property/{{ Session::get('property_uuid') }}/tenant/{{ $contract->tenant_uuid }}/contract/{{ $contract->uuid }}/moveout/step-5"
                                class="group">
                                <span
                                    class="absolute top-0 left-0 h-full w-1 bg-transparent group-hover:bg-gray-200 lg:bottom-0 lg:top-auto lg:h-1 lg:w-full"
                                    aria-hidden="true"></span>
        ...
        <div class="mt-5 px-4 sm:px-6 lg:px-8">
            {{-- start-step-3-form --}}
            @livewire('unit-inventory-component', ['property' => $property,'unitDetails' => $contract->unit, 'tenant' =>
            $contract->tenant, 'contract' => $contract, 'ismovein' => $ismovein])
