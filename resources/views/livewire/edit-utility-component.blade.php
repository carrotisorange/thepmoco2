<x-modal-component>
    ...
    <div class="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
        <div class="relative transform overflow-hidden rounded-lgpx-4 pt-5 pb-4 text-left transition-all sm:my-8 sm:w-full sm:max-w-sm sm:p-6">
            ...
            <form wire:submmit.prevent="updateUtility">
                ...
                <div class="mt-5 sm:mt-6">
                    ...
                    <option value="{{ $tenant->tenant_uuid }}" {{ $tenant->tenant_uuid===$tenant_uuid? 'selected' : 'Select one' }}>
                        {{ $tenant->tenant->tenant }} (T)
                    </option>
                    @endforeach
                    @foreach($owners as $owner)
                        <option value="{{ $owner->owner_uuid }}" {{ $owner->owner_uuid===$owner_uuid? 'selected' : 'Select one' }}>
                            {{ $owner->owner->owner }} (O)
                        </option>
                        @endforeach

                         {{-- @foreach($guests as $guest)
                        <option value="{{ $guest->guest_uuid }}" {{ $guest->guest_uuid===$guest_uuid? 'selected' : 'Select one' }}>
                            {{ $guest->guest->guest }} (G)
                        </option>
                        @endforeach --}}
                    </x-form-select>
                ...
                <div class="mt-5 sm:mt-6">
                    ...
                    <button type="button" wire:click="updateUtility" wire:loading.remove
                        class="inline-flex w-full justify-center rounded-md border border-transparent bg-purple-500 px-4 py-2 text-base font-medium text-white shadow-sm hover:bg-purple-700 focus:outline-none focus:ring-2 focus:ring-purple-500 focus:ring-offset-2 sm:text-sm">
                        <i class="fa-solid fa-arrow-right"></i>&nbsp Update
                    </button>
                    <button type="button" wire:loading wire:target="updateUtility" disabled
                        class="inline-flex w-full justify-center rounded-md border border-transparent bg-purple-500 px-4 py-2 text-base font-medium text-white shadow-sm hover:bg-purple-700 focus:outline-none focus:ring-2 focus:ring-purple-500 focus:ring-offset-2 sm:text-sm">
                        Loading...
                    </button>
                </div>
