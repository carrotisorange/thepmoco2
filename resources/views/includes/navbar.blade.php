        ...
        @foreach($availableFeatures as $feature)
            @if($feature->feature->is_active)
                ...
                <x-nav-link href="{{ $routeToTheFeature }}" :active="request()->routeIs($feature->feature->alias)">
                    ...
                    <img title="{{ $feature->feature->feature }}" class="h-8 w-auto" src="{{ asset('/brands/'.$feature->feature->default_icon) }}" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" aria-hidden="true">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M20 13V6a2 2 0 00-2-2H6a2 2 0 00-2 2v7m16 0v5a2 2 0 01-2 2H6a2 2 0 01-2-2v-5m16 0h-2.586a1 1 0 00-.707.293l-2.414 2.414a1 1 0 01-.707.293h-3.172a1 1 0 01-.707-.293l-2.414-2.414A1 1 0 006.586 13H4" />
                </x-nav-link>

                <p class="font-medium leading-3 ml-0 text-xs text-gray-900 mt-10">{{ $feature->feature->feature }}</p>
            @endif
        @endforeach
    </div>
</nav>
