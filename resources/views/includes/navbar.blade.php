    ...
    <div class="relative flex w-22 flex-col space-y-3 p-3">
        ...
        @foreach(App\Models\UserRestriction::where('user_id',auth()->user()->id)->where('property_uuid', Session::get('property_uuid'))->where('restriction_id', 2)->where('is_approved',1)->groupBy('feature_id')->orderBy('feature_id', 'asc')->get() as $feature)
            @if($feature->feature->is_active)
                ...
                <x-nav-link href="/property/{{ Session::get('property_uuid') }}/{{ $feature->feature->alias }}"
                    :active="request()->routeIs($feature->feature->alias)">
                    <span class="sr-only">{{ $feature->feature->feature }}</span>
                    <img class="h-8 w-auto" src="{{ asset('/brands/'.$feature->feature->default_icon) }}" fill="none"
